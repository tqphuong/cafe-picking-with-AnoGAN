...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.prob_real = tf.nn.sigmoid(self.D_logits)
    self.prob_fake = tf.nn.sigmoid(self.D_logits_)
    #summary op.
  ...
  def train(self, config):
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, batch_idxs):
        ...
        a=self.sess.run(self.prob_real,feed_dict = sample_feed_dict)
        count_a=0
        for i in range(len(a)):
          if a[i]>=0.5:
            count_a+=1
        b=self.sess.run(self.prob_fake,feed_dict = sample_feed_dict)
        count_b=0
        for i in range(len(a)):
          if b[i]<0.5:
            count_b+=1
        real_acc=count_a/config.batch_size
        fake_acc=count_b/config.batch_size
        ...
        total_loss_dis += errD_fake+errD_real
        total_loss_gen += errG
        total_acc_real += real_acc
        total_acc_fake += fake_acc
        counter += 1
        print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f,acc_real: %.8f,acc_fake: %.8f" \
          % (epoch, idx, batch_idxs, time.time() - start_time, errD_fake+errD_real, errG,real_acc,fake_acc))
        if np.mod(counter, 100) == 1:
          ...
          print("[Sample] d_loss: %.8f, g_loss: %.8f,d_loss_1: %.8f, g_loss_1: %.8f " % (d_loss, g_loss,errD_fake+errD_real, errG)) 
        if np.mod(counter, 500) == 2:
          self.save(config.checkpoint_dir, counter)
      loss_curve_dis.append(total_loss_dis/(batch_idxs))
      loss_curve_gen.append(total_loss_gen/(batch_idxs))
      acc_curve_real.append(total_acc_real/(batch_idxs))
      acc_curve_fake.append(total_acc_fake/(batch_idxs))
      epoch_list.append(epoch)
      print("Epoch %02d - Loss of Dis:%f - Loss of Gen:%f"
              %(epoch, loss_curve_dis[-1], loss_curve_gen[-1]))
      print("         - Accuracy of Real:%f - Accuracy of Fake:%f"
              %(acc_curve_real[-1], acc_curve_fake[-1]))
    plt.plot(epoch_list, loss_curve_dis, label = "dis_loss")
    plt.plot(epoch_list,loss_curve_gen,label="gen_loss") 
    plt.xlabel('epoch')
    plt.ylabel('Loss') 
    plt.title('Learning curve')
    plt.legend()
    plt.show()
    plt.plot(epoch_list,acc_curve_real, label = "real_acc")
    plt.plot(epoch_list,acc_curve_fake,label="fake_acc") 
    plt.xlabel('epoch')
    plt.xlabel('Accuracy') 
  ...
  def _sampler(self, z, y=None, batch_size = None):
    ...
    with tf.variable_scope("generator") as scope:
      ...
      if not self.y_dim:
        ...
        # yb = tf.reshape(y, [-1, 1, 1, self.y_dim])
        print(y,[self.batch_size, 1, 1, self.y_dim])
        yb = tf.reshape(y, [self.batch_size, 1, 1, self.y_dim])
        z = concat([z, y], 1)
  ...
  def train_anomaly_detector(self, config, test_data, test_data_name):
    ...
    for epoch in range(config.test_epoch):
      ...
      if epoch==config.test_epoch-1:
          f.write(str(ano_score) + '\n')
      if epoch in save_epoch:
        ...
        filename[0]=filename[0].replace('x_', 'x_hat_')
        imageio.imwrite(os.path.join(path,filename[0]),samples)
        #imageio.imwrite(os.path.join(path,filename[1]),errors)
